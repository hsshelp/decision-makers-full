class Scholar < User
  ...
  def asset_analysis(profile)
    ...
    questions.each do |question|
      ...
      asset_analysis << OpenStruct.new({question:"#{question.question_number}. #{question.question}", response:response.response.to_i, color:question.question_category.try(:color)})
    end
  ...
  def asset_cluster_analysis(profile)
    ...
    QuestionCategory.all.each do |question_category|
      ...
      asset_cluster_analysis << OpenStruct.new({:category => question_category.name, :value => responses.collect{|r| r.response.to_i}.inject(:+)/num_responses,  :color => question_category.try(:color)})
    end
  ...
  def reflective_decision_making(profile)
    ...
    snapshot_2_q = profile.snapshot_2.try(:questions) || []
    snapshot_4_qa = profile.snapshot_4.try(:questions).try(:slice, 1, 2) || []
    snapshot_4_qb = profile.snapshot_4.try(:questions).try(:slice, 4, 5) || []
    questions = snapshot_2_q + snapshot_4_qa + snapshot_4_qb
